...
if [[ "$MODE" == "aws" ]]; then
  ...
  if [[ -z "$EC2_HOST" ]]; then
    ...
    echo "     curl -fsSL https://raw.githubusercontent.com/cryptostoner94/super-agent-os/main/super-agent-os/MASTER_DEPLOY.sh -o MASTER_DEPLOY.sh"
  ...
  rsync -az --exclude='.git' --exclude='data/' --exclude='__pycache__' \
    ...
    . "$EC2_USER@$EC2_HOST:~/super-agent-os/"

  # Execute deploy on remote
  ssh -i "$EC2_KEY" -o StrictHostKeyChecking=no "$EC2_USER@$EC2_HOST" \
    "cd ~/super-agent-os && bash MASTER_DEPLOY.sh --local"
