...
async def _hf_keepalive_loop():
    """Touch the event DB every 25 min to prevent HF Space from hitting the 48h inactivity sleep."""
    await asyncio.sleep(90)
    while True:
        try:
            await log_event("keepalive", {"status": "alive"})
        except Exception:
            pass
...
try:
    from backend.app.api.routes_upgrade import router as _upgrade_router
    app.include_router(_upgrade_router)
except Exception:
    pass

try:
    from backend.app.api.routes_execute import router as _execute_router
